...
// Logging and events trigger.
        ...
        $htmlprint .= $OUTPUT->header();

        if (empty($cm->visible) and !has_capability('mod/facetoface:viewemptyactivities', $context)) {
            notice(get_string('activityiscurrentlyhidden'));
        }
        $htmlprint .= $OUTPUT->box_start();
        $htmlprint .= $OUTPUT->heading(get_string('allsessionsin', 'facetoface', format_string($facetoface->name)), 2);

        if ($facetoface->intro) {
            $htmlprint .= $OUTPUT->box_start('generalbox', 'description');
            $htmlprint .= format_module_intro('facetoface', $facetoface, $cm->id);
            $htmlprint .= $OUTPUT->box_end();
        } else {
            $htmlprint .= html_writer::empty_tag('br');
        }
        $locations = get_locations($facetoface->id);
        if (count($locations) > 2) {
            $htmlprint .= html_writer::start_tag('form', array('action' => 'view.php', 'method' => 'get', 'class' => 'formlocation'));
            $htmlprint .= html_writer::start_tag('div');
            $htmlprint .= html_writer::empty_tag('input', array('type' => 'hidden', 'name' => 'f', 'value' => $facetoface->id));
            $htmlprint .= html_writer::select($locations, 'location', $location, '', array('onchange' => 'this.form.submit();'));
            $htmlprint .= html_writer::end_tag('div'). html_writer::end_tag('form');
        }

        print_session_list($course->id, $facetoface, $location);

        if (has_capability('mod/facetoface:viewattendees', $context)) {
            $htmlprint .= $OUTPUT->heading(get_string('exportattendance', 'facetoface'));
            $htmlprint .= html_writer::start_tag('form', array('action' => 'view.php', 'method' => 'get'));
            $htmlprint .= html_writer::start_tag('div');
            $htmlprint .= html_writer::empty_tag('input', array('type' => 'hidden', 'name' => 'f', 'value' => $facetoface->id));
            $htmlprint .= get_string('format', 'facetoface') . '&nbsp;';
            $formats = array('excel' => get_string('excelformat', 'facetoface'),
                'ods' => get_string('odsformat', 'facetoface'));
            $htmlprint .= html_writer::select($formats, 'download', 'excel', '');
            $htmlprint .= html_writer::empty_tag('input', array('type' => 'submit', 'value' => get_string('exporttofile', 'facetoface')));
            $htmlprint .= html_writer::end_tag('div'). html_writer::end_tag('form');
        }

        $htmlprint .= $OUTPUT->box_end();
        $htmlprint .= $OUTPUT->footer($course);

        function print_session_list($courseid, $facetoface, $location) {
            global $CFG, $USER, $DB, $OUTPUT, $PAGE, $htmlprint;
            ...
            // Upcoming sessions.
            $htmlprint .= $OUTPUT->heading(get_string('upcomingsessions', 'facetoface'));
            ...
            if (!empty($upcomingarray) && $bulksignup) {
                ...
                $htmlprint .= html_writer::tag('p', $signupforstreamlink);
            }
            if (empty($upcomingarray) && empty($upcomingtbdarray)) {
                ...
                $htmlprint .= $f2frenderer->print_session_list_table($customfields, $upcomingarray, $viewattendees, $editsessions, !$bulksignup);
            }
            ...
            if ($editsessions) {
                ...
                $htmlprint .= html_writer::tag('p', $addsessionlink);
            }

            // Previous sessions.
            if (!empty($previousarray)) {
                $htmlprint .= $OUTPUT->heading(get_string('previoussessions', 'facetoface'));
                $htmlprint .= $f2frenderer->print_session_list_table($customfields, $previousarray, $viewattendees, $editsessions);
            }
        }
